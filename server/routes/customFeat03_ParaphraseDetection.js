// Custom Feature: Paraphrase detection
// Original text + suspected plagiarism; AI detects paraphrases not caught by string matching; highlights suspicious sections
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPlagiarismAIContentDetector / cf-paraphrase-detection',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_paraphrase_detection`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-paraphrase-detection feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Paraphrase detection'.
Context: Original text + suspected plagiarism; AI detects paraphrases not caught by string matching; highlights suspicious sections
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-paraphrase-detection/run error:', err);
