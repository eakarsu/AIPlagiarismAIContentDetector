// Custom Feature: AI-generated content detection ensemble
// Multiple detectors (OpenAI, Turnitin, custom ML); ensemble voting; generates confidence scores with reasoning
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPlagiarismAIContentDetector / cf-ai-generated-content-detection-ensemble',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_ai_generated_content_detection_ensemble`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-ai-generated-content-detection-ensemble feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'AI-generated content detection ensemble'.
Context: Multiple detectors (OpenAI, Turnitin, custom ML); ensemble voting; generates confidence scores with reasoning
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-ai-generated-content-detection-ensemble/run error:', err);
