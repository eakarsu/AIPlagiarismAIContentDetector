// Custom Feature: Agentic plagiarism monitoring
// Continuous background process monitors submissions; auto-generates plagiarism reports; flags suspicious patterns (same passages, multiple students); notifies instructor
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPlagiarismAIContentDetector / cf-agentic-plagiarism-monitoring',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_agentic_plagiarism_monitoring`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-agentic-plagiarism-monitoring feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Agentic plagiarism monitoring'.
Context: Continuous background process monitors submissions; auto-generates plagiarism reports; flags suspicious patterns (same passages, multiple students); notifies instructor
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-agentic-plagiarism-monitoring/run error:', err);
