...
// Gap Feature: Only 7 frontend pages for 16
// Only 7 frontend pages for 16-route backend
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPlagiarismAIContentDetector / gap-only-7-frontend-pages-for-16',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Only 7 frontend pages for 16'.\nProject: AIPlagiarismAIContentDetector.\nContext: Only 7 frontend pages for 16-route backend\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['only-7-frontend-pages-for-16', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'only-7-frontend-pages-for-16', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
