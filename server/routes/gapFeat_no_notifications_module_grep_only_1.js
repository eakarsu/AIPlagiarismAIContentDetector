...
// Gap Feature: No notifications module (grep only 1)
// No notifications module (grep only 1)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPlagiarismAIContentDetector / gap-no-notifications-module-grep-only-1',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No notifications module (grep only 1)'.\nProject: AIPlagiarismAIContentDetector.\nContext: No notifications module (grep only 1)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-notifications-module-grep-only-1', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-notifications-module-grep-only-1', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
