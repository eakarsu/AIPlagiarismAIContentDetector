...
// Gap Feature: Limited analytics (trends across courses/cohorts)
// Limited analytics (trends across courses/cohorts)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPlagiarismAIContentDetector / gap-limited-analytics-trends-across-courses-cohorts',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Limited analytics (trends across courses/cohorts)'.\nProject: AIPlagiarismAIContentDetector.\nContext: Limited analytics (trends across courses/cohorts)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['limited-analytics-trends-across-courses-cohorts', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'limited-analytics-trends-across-courses-cohorts', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
