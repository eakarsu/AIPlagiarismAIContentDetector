// Custom Feature: Writing style fingerprinting
// Build model of student\'s authentic writing style; flag submissions that diverge significantly (likely AI-written or plagiarized)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPlagiarismAIContentDetector / cf-writing-style-fingerprinting',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_writing_style_fingerprinting`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-writing-style-fingerprinting feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Writing style fingerprinting'.
Context: Build model of student\'s authentic writing style; flag submissions that diverge significantly (likely AI-written or plagiarized)
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-writing-style-fingerprinting/run error:', err);
