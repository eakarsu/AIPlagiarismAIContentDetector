// Custom Feature: Student writing improvement
// Flagged writing issues (grammar, clarity, citation format); AI suggests improvements; integrate feedback directly into document
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPlagiarismAIContentDetector / cf-student-writing-improvement',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_student_writing_improvement`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-student-writing-improvement feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Student writing improvement'.
Context: Flagged writing issues (grammar, clarity, citation format); AI suggests improvements; integrate feedback directly into document
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-student-writing-improvement/run error:', err);
